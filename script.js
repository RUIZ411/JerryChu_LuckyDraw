...
function generateFromSettings(){const s=getDraftSettings(),err=validateSettings(s);if(err){updateSettingsSummary();return}if(state.session?.active){alert("진행 중인 후원 수신을 먼저 종료한 뒤 새 뽑기판을 생성해 주세요.");return}if(state.board.some(x=>x.opened)&&!confirm("새 뽑기판을 만들면 현재 진행 기록과 열린 칸이 모두 초기화됩니다. 계속할까요?"))return;state.settings=s;buildBoard();restartPolling();closeModal("settingsModal")}
